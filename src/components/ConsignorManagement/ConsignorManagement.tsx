...
const ConsignorManagement = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setIsLoading(true);
      try {
        const response = await fetchConsignors();
        setData(response);
  ...
  const handleSubmit = async (formData: any) => {
    ...
    if (selectedData && selectedData._id) {
      // Update an existing consignor - ensure we're using the _id from selectedData
      try {
        const updatedData = {
          ...formData,
        ...
        const updated = await updateConsignor(selectedData._id, updatedData);
        ...
        alert("Error updating consignor. Please try again.");
      }
    } else {
      // Create a new consignor
      try {
        const created = await createConsignor(formData);
        setData((prev) => [...prev, created]);
        setIsModalOpen(false); // Close the add modal after creating
      } catch (error) {
        console.error("Error creating consignor:", error);
        alert("Error creating consignor. Please try again.");
      }
    }
  };

  const handleDelete = async (id: string) => {
    const success = await deleteConsignor(id);
    if (success) {
      setData((prev) => prev.filter((consignor) => consignor._id !== id));
      ...
      Object.keys(obj).forEach((key) => {
        ...
        if (key === "Documents") {
          ...
          const field = {
            id: fieldName,
            name: fieldName,
            placeholder: `Consignor ${key.replace(/([A-Z])/g, " $1").toLowerCase()}`,
          ...
          >
            ...
            >
              ...
              <p className="mb-6 text-gray-600">
                Are you sure you want to delete this consignor? This action
                cannot be undone.
              </p>
      ...
      >
        <div className="flex flex-col justify-between gap-6 sm:flex-row sm:items-center">
          <div>
            ...
            >
              Manage all your consignors in one place
            </motion.p>
          </div>
                  ...
                  <tr>
                    ...
                    >
                      <motion.div
                        ...
                        transition={{ duration: 0.5 }}
                      >
                        <p className="text-lg">No consignors found</p>
                        <p className="mt-2 text-sm">
                          Try adjusting your search or add a new consignor
                        </p>
                      </motion.div>
                    </td>
                  </tr>
